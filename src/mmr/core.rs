...
impl Mmr {
    ...
    pub fn create_from_peaks(
        ...
        let current_elements_count = mmr.get_elements_count()?;
        ...
        mmr.set_leaves_count(leaves_count)?;
        mmr.set_elements_count(elements_count)?;

        for (peak_index, peak_hash) in expected_peak_indices.iter().zip(peaks_hashes.iter()) {
            mmr.set_node_hash(*peak_index, *peak_hash)?;
        }

        let bag = mmr.bag_the_peaks(Some(elements_count))?;
        let root_hash = mmr.calculate_root_hash(&bag, elements_count)?;
        mmr.set_root_hash(root_hash)?;
    ...
    pub fn append(&mut self, value: Hash32) -> Result<AppendResult, MmrError> {
        let batch_result = self.batch_append(&[value])?;
        Ok(AppendResult {
    ...
    pub fn batch_append(&mut self, values: &[Hash32]) -> Result<BatchAppendResult, MmrError> {
        ...
        let append_state = self.prepare_append_state()?;
        ...
        self.store.set_many(staged_writes)?;
    ...
    pub fn get_proof(
        ...
        let tree_size = elements_count.unwrap_or(self.get_elements_count()?);
        ...
        let sibling_values = self.store.get_many(&sibling_keys)?;
    ...
    pub fn get_root_hash(&self) -> Result<Option<Hash32>, MmrError> {
        match self.store.get(&self.root_hash_key())? {
            Some(value) => Ok(Some(value.expect_hash(&self.root_hash_key())?)),
            None => Ok(None),
        }
    }

    fn retrieve_peaks_hashes(&self, peak_idxs: Vec<u64>) -> Result<Vec<Hash32>, MmrError> {
    ...
    fn prepare_append_state(&mut self) -> Result<AppendState, MmrError> {
        let cached_counts = self.load_cached_counts()?;
        ...
        let append_state = self.load_append_state(&peak_indices)?;
    ...
    fn load_cached_counts(&mut self) -> Result<CachedCounts, MmrError> {
    ...
    fn load_append_state(&self, peak_indices: &[u64]) -> Result<AppendState, MmrError> {
        let leaf_count_key = self.leaf_count_key();
        let elements_count_key = self.elements_count_key();
    ...
    pub fn get_leaves_count(&self) -> Result<u64, MmrError> {
        match self.store.get(&self.leaf_count_key())? {
            Some(value) => Ok(value.expect_u64(&self.leaf_count_key())?),
            None => Ok(0),
        }
    }

    fn set_leaves_count(&self, value: u64) -> Result<(), MmrError> {
        self.store
    ...
    pub fn get_elements_count(&self) -> Result<u64, MmrError> {
        match self.store.get(&self.elements_count_key())? {
            Some(value) => Ok(value.expect_u64(&self.elements_count_key())?),
            None => Ok(0),
        }
    }

    fn set_elements_count(&self, value: u64) -> Result<(), MmrError> {
        self.store
            .set(self.elements_count_key(), StoreValue::U64(value))
            .map_err(MmrError::from)
    }

    fn set_root_hash(&self, hash: Hash32) -> Result<(), MmrError> {
        self.store
            .set(self.root_hash_key(), StoreValue::Hash(hash))
            .map_err(MmrError::from)
    }

    fn get_node_hash(&self, index: u64) -> Result<Option<Hash32>, MmrError> {
    ...
    fn set_node_hash(&self, index: u64, hash: Hash32) -> Result<(), MmrError> {
        self.store
